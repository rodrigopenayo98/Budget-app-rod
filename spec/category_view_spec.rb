...
RSpec.describe 'categories/index.html.erb', type: :view do
  it 'presents a list of categories and provides a link to add a new category' do
    user = User.create(name: 'User', email: 'user@user.com', password: 'password')

    category1 = Category.create(name: 'Category1', icon: 'Icon1', user:)
    category2 = Category.create(name: 'Category2', icon: 'Icon2', user:)
...
RSpec.describe 'categories/new.html.erb', type: :view do
  ...
  it 'renders the form for adding a new category' do
    render
